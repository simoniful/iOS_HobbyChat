...
final class HomeChatViewController: BaseViewController {
  private lazy var detailBarButton = UIBarButtonItem(
  ...
  private let headerView = HomeChatHeaderView()
  ...
  private lazy var input = HomeChatViewModel.Input(
    ...
    output.chatList
      .drive(tableView.rx.items) { tv, index, chat in
        if chat.from == "aV43LR1IKjUenCcqFIX44lQHUlz1" {
          let cell = tv.dequeueReusableCell(withIdentifier: HomeChatSendCell.identifier) as! HomeChatSendCell
          cell.updateUI(chat: chat)
          return cell
        } else {
          let cell = tv.dequeueReusableCell(withIdentifier: HomeChatReceiveCell.identifier) as! HomeChatReceiveCell
          cell.updateUI(chat: chat)
          return cell
        }
      }
  ...
  private func setupAttributes() {
    ...
    tableView.register(HomeChatHeaderView.self,
                       forHeaderFooterViewReuseIdentifier: HomeChatHeaderView.identifier)
    tableView.register(HomeChatSendCell.self,
                       forCellReuseIdentifier: HomeChatSendCell.identifier)
    tableView.register(HomeChatReceiveCell.self,
                       forCellReuseIdentifier: HomeChatReceiveCell.identifier)
